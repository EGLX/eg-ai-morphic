...
export function createToolCallingStreamResponse(config: BaseStreamConfig & { agent?: string }) {
    return createDataStreamResponse({
        execute: async (dataStream: DataStreamWriter) => {
            const { messages, model, chatId, searchMode, userId, agent } = config
            ...
            try {
                ...
                // Use dune agent if specified, otherwise use default researcher
                let researcherConfig
                if (agent === 'dune') {
                    researcherConfig = await agentDune({
